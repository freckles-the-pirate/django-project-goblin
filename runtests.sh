...
BASE_TESTING_PATH=$1
BASE_TESTING_MODULE=$(echo $BASE_TESTING_PATH | sed 's/\.\///g' | sed \
's/\//\./g')
...
if [ -n $(which django-admin.py) ] ; then
...
# TESTS=$(find $BASE_TESTING_PATH -print | grep -P --color=never 
#'tests/test_.+\.py$')
